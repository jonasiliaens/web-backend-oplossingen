<?php
	...
	$inhoudTxt 		= 	file_get_contents('opdracht-cookies-deel4.txt');

	$arrayInhoudTxt = 	explode('#', $inhoudTxt);

	$inhoudTxtMulti = 	array();
	...
	foreach ($arrayInhoudTxt as $key => $value) 
	{
		$inhoudTxtMulti [] = explode(',', $value);
	}
	...
	if (isset($_POST[ 'verzenden' ]))
	{
		...
		$id = array_search($username, $inhoudTxtMulti);
		var_dump($inhoudTxtMulti);
		var_dump($id);
